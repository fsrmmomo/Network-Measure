...
def base2(routing):
    ...
    sw_node = get_sw_node_list()
    node_num = 1 + 1 + len(sw_node) * 2 + task_num * len(routing)
    sort_res = sort_task_and_subflow1(routing)
    adj, adj_copy= build_graph(capacity, task_num, node_num, sw_node, routing)
    ans = max_flow(adj, task_num, node_num, sw_node, routing, sort_res)
    print(ans)
    return get_ans(adj, adj_copy, task_num, node_num, sw_node, routing)
def base3(routing):
    ...
    for t in tasks:
        t.remain = len(routing)
    sw_node = get_sw_node_list()
    ratio = sort_task_and_subflow(routing)
    task_num = 1
    node_num = 1 + 1 + len(sw_node) * 2 + task_num * len(routing)
    adj, adj_copy = build_graph(capacity, task_num, node_num, sw_node, routing)
    ans = max_flow(adj, task_num, node_num, sw_node, routing, [])
    print(ans)
    return get_ans3(adj, adj_copy, task_num, node_num, sw_node, routing,ratio)
...
def max_flow(adj, task_num, node_num, sw_node, routing, sort_res):
    ...
    # print('111')
    return ans
def get_ans3(adj, adj_copy, task_num, node_num, sw_node, routing,ratio):
    # print(node_num)
    ...
    for taskID in range(task_num):
        for i, r in enumerate(routing):
            ...
            for k, sw in enumerate(sw_node):
                tot = adj[node_find[sw]][f][0] / tasks[taskID].usage[0]
                for rr in ratio:
                    ans += int(tot * rr)
...
def sort_task_only(routing):
    ...
    tasks.sort(key=lambda x: x.weight)
    return []
def sort_task_and_subflow1(routing):
    if len(tasks) == 1:
        result = []
        for i in range(len(routing)):
            result.append([0, i])
...
def m_task_m_resource(mode="00000"):
    ...
    if mode[0] == "1":
        ...
        for i in range(10):
            ...
            resource = i+1
            # 流和c不变
            nc = [[x[0] for _ in range(i+1)] for x in new_clist[2]]
            # print(nc)
            set_capacity(capacity, nc) # 使用总资源0.6比例，更好观察效果
            set_routing(routing, new_fd_list)
            tmp = []

            for j in range(times):
                print(str(i)+" "+str(j))
                new_tasks = tt[4][i+1][j]
                tmp.append(compute2(nc,new_fd_list,resource,routing,new_tasks,mm))
            result.append(tmp)
        print(result)
        plain_save("../../result/mm-1"+str(mm), result)
        # plain_save("../../result/mm-1"+str(mm), result)

    # 使用四个任务，四个资源，相同流和相同分布，跟随C变化
    # global resource
    resource = 4
    if mode[1] == "1":
        result = []
        ...
        new_fd_list = get_same_f()
        for i in range(10):
            nc = [[x[0] for _ in range(4)] for x in new_clist[i]]
            ...
            result.append(tmp)
        print(result)
        plain_save("../../result/mm-2"+str(mm), result)

    # 使用四个任务，四个资源，流相同，分布不同，跟随C变化
    if mode[2] == "1":
        result = []
        new_clist = get_step_clist_diff_node()
        new_fd_list = get_same_f()
        for i in range(10):
            for k in range(1):
                nc = [[x[0] for _ in range(4)] for x in new_clist[i][k]]
                tmp = []
                for j in range(times):
                    print(str(i)+" "+str(j))
                    new_tasks = tt[4][4][j]
            ...
            result.append(tmp)
        print(result)
        plain_save("../../result/mm-3"+str(mm), result)

    # 使用四个任务，四个资源，流不同，分布相同，跟随C变化
    if mode[3] == "1":
        result = []
        new_clist = get_step_clist_same_node()
        new_fd_list = get_diff_f()
        for i in range(10):
            ...
            for k in range(1):
                ...
                for j in range(times):
                    print(str(i)+" "+str(j))
                    new_tasks = tt[4][4][j]
                    tmp.append(compute2(nc, new_fd_list[k], resource, routing, new_tasks, mm))
            result.append(tmp)
        print(result)
        plain_save("../../result/mm-4"+str(mm), result)


    # 使用四个任务，四个资源，流不同，分布不同，跟随C变化
    if mode[4] == "1":
        result = []
        new_clist = get_step_clist_diff_node()
        new_fd_list = get_diff_f()
        ...
        for i in range(10):
            nc = [[x[0] for _ in range(4)] for x in new_clist[i][0]]
            for k in range(1):
                tmp = []
                for j in range(times):
                    print(str(i)+" "+str(j))
            ...
            result.append(tmp)
        print(result)
        plain_save("../../result/mm-5"+str(mm), result)
...
def plain_save(filename,result):
    with open(filename, "w") as f:
        for r in result:
            f.write(str(r))
            f.write('\n')
def compute2(new_clist,new_fd_list,resource,routing,new_tasks,mode = 1):
    global capacity
    set_capacity(capacity, new_clist)
    set_routing(routing,new_fd_list)
    ...
    if mode == 1:
        ...
        tasks.extend(new_tasks)
        # 方法1 基于总资源排序
        c_ans1 = base1(routing)

        tasks.clear()
        tasks.extend(new_tasks)
        c_ans2 = base2(routing)
        print(c_ans1)
        print(c_ans2)
        return [c_ans1,c_ans2]
    else:
        tasks.clear()
        tasks.extend(new_tasks)
        lp_ans = LP_algo_integer(routing, capacity, resource)
        ...
        tasks.clear()
        tasks.extend(new_tasks)
        c_ans3 = base3(routing)

        print(c_ans3)
        print(lp_ans)
        print(c_ans3 / lp_ans)
        return [c_ans3,lp_ans]
